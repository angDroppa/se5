...
export const LoginSchema = z.object({
  email: z.email("Inserisci una email valida").openapi({ example: "mario@example.com" }),
  password: z.string()
...
export const RegisterSchema = z.object({
  ...
  lastName: z.string().trim().min(1, "Il cognome è obbligatorio").openapi({ example: "Rossi" }),
  email: z.email("Inserisci una email valida").openapi({ example: "mario@example.com" }),
  password: z.string()
    .min(8, "Deve contenere almeno 8 caratteri")
    .regex(/[A-Z]/, "Deve contenere almeno una lettera maiuscola")
